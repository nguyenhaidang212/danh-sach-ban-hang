...
$.addEventListener("click", (e) => {
    ...
    getApi().then((data) => {
      ...
      if (arrList.length == 0) {
        ...
        arrList.forEach((order) => {
          ...
          document.querySelector(".order-detail-body").insertAdjacentHTML("afterbegin",
          ...
          `)
          order.item.forEach(value => {
            document.querySelector(".order-detail-list").insertAdjacentElement("beforeend", `
            <div class="detail-list-title">
            <span></span>
            <span>Product</span>
            <span>Quantity</span>
  ...
  if (e.target.matches(".order-detail-overlay")&&!e.target.matches(".order-detail-body")) {
