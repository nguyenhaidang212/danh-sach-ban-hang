...
function renderOrderLists1(value) {
  apiOrders.forEach((order) => {
    let count = 0;
    order.item.forEach((e) => (count += countItemBuy));
    document.querySelector(".order-grid").insertAdjacentHTML(
      "beforeend",
      `
    ...
    <span id="username" class="order-detail"> ${order.info.username}</span>
        ...
        <span class="order-detail"> ${order.item.reduce()}$</span>
