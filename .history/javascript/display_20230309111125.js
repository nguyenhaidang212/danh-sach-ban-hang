...
$.addEventListener("click", (e) => {
  if (e.target.matches(".btn-search")) {
    document.querySelectorAll(".order-gird-items").forEach((e) => e.remove());
    ...
    getApi().then((data) => {
      ...
      apiOrders.forEach((order) => {
        if (order.info.name == value || order.info.orderNumber == value) {
          arrList.push(order);
        }
      });
      if (arrList.length == 0) {
        ...
        arrList.forEach((order) => {
          document.querySelector(".order-gird").insertAdjacentHTML(
            "afterend",
            `<div class="order-gird-items">
            <span>${order.info.orderNumber}</span><span>${order.info.username}</span
          ><span>xu ly</span><span>${order.total}</span><span>${order.info.date}</span
          ><span class="show-detail">...</span></div>`
          );
        });
      }
    });
