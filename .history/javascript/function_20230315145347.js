...
function createOrder() {
  ...
  if (
    ...
    const orderUser = {
      orderNumber: Math.floor(Math.random() * 3),
      date:
        date.getFullYear() +
        "/" +
  ...
  list.forEach((e) => {
    listItem.forEach((value) => {
      if (value.name == e.name) {
        e.so_luong += value.so_luong_mua;
        e.so_luong_mua -= value.so_luong_mua;
      }
    });
  });
...
function delItemBuy(name, value) {
  ...
  newList.forEach((e, index) => {
    if (e.name == name) {
      list.forEach((value) => {
        if (value.name == e.name) {
          value.so_luong_mua -= e.so_luong_mua;
...
function checkID() {
  ...
  if (arrayID.includes(order.orderNumber)) {
    console.log(1);
    order.orderNumber += Math.floor(Math.random() * 3);
    setOrder(order);
    checkID();
  } else {
    console.log(2);
