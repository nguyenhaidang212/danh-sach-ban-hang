...
$.addEventListener("click", (e) => {
  ...
  if (e.target.matches(".plus-icon")) {
    ...
    getListLocalstorage().forEach((value) => {
      ...
      if (value.so_luong == 0) {
        document.querySelectorAll(".buy-name").forEach((f) => {
          if (
            f.textContent ==
            e.target.parentNode.previousElementSibling.textContent
          ) {
            console.log(f.nextElementSibling.hasChildNodes(".plus-icon"));
            // const child = document.querySelector(`.${value.name}`);
            f.nextElementSibling.removeChild(".plus-icon");
          }
        });
      }
      listNew.push(value);
