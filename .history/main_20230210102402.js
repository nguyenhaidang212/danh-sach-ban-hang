...
$.addEventListener("click", (e) => {
  ...
  //------Tang giam------
  if (e.target.matches(".minus-icon") || e.target.matches(".plus-icon")) {
    console.log(1);
    plusItem(e.target);
  }
});
...
function plusItem(value) {
  console.log(value.parentNode.previousElementSibling.textContent);
  arrayItemAdd.forEach((e) => {
    if (e.name == value.parentNode.previousElementSibling.textContent) {
      value.parentNode.previousElementSibling.remove();
      e.quality++;
      displayItem();
    }
  });
