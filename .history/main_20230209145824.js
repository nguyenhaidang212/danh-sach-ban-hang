...
buyDisplay.addEventListener("click", (e) => {
  ...
  const delItem = JSON.parse(localStorage.getItem(keyLocalStorageItemCart));
  ...
  buyDel.forEach((e) => {
    e.addEventListener("click", (value) => {
      ...
      const nameItemDel =
        value.target.parentNode.previousElementSibling.previousElementSibling
          .previousElementSibling.previousElementSibling;
      const newDelItem = delItem.forEach((e, index) => {
        if (e.name == nameItemDel.textContent) {
          delItem.splice(index, 1);
        }
      });
      localStorage.setItem(keyLocalStorageItemCart, JSON.stringify(newDelItem));
      console.log(JSON.parse(localStorage.getItem(keyLocalStorageItemCart)));
    });
  });
});
