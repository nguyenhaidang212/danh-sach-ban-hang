...
$.addEventListener("click", (e) => {
  ...
  if (e.target.matches(".plus-icon")) {
    ...
    //   Number(e.target.parentNode.previousElementSibling.textContent) + 1;
    total();
    countItem();
    let listNew = [];
    getListLocalstorage().forEach((value) => {
      if (
        ...
        value.so_luong -= 1;
        if (value.so_luong == 0) {
          document.querySelectorAll(".plus-icon").forEach((f) => {
            if (f.parentNode.previousElementSibling.textContent == value.name) {
              f.remove();
            }
          });
        }
